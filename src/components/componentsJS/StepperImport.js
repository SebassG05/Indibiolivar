...
export default function HorizontalLinearStepperImport({ onSubmit }) {
  ...
  const fetchTables = async () => {
    try {
      const response = await fetch('http://localhost:5002/api/tables'); // Endpoint para obtener enfermedades disponibles
  ...
  const uploadFile = () => {
    ...
    axios.post('http://localhost:5002/api/upload', formData, {
      headers: {
